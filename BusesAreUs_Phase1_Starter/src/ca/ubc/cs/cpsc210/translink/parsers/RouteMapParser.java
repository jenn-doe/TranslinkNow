...
public class RouteMapParser {
    ...
    private void parseOnePattern(String str) {
        //list to keep track of latlon. use regex, substring, indexof.
        //indexOf(char ch, int startIndex) returns index of where the character is
        // String substring(int startIndex, int endIndex), given start and end, cuts out and returns that part of the string, 0-based, includes first not last
        // and String[] split(String regex)
        List<LatLon> listOfLatLon = new ArrayList<>();
        String routeNumber;
        String patternName;


        routeNumber = str.substring(1, str.indexOf("-", 1));
        patternName = str.substring(str.indexOf("-", 1) + 1, str.indexOf(";"));
        ...
        for (int i = 1; i < stringArray.length; i++) {
            ...
            if (i % 2 == 0) {
                lon = Double.parseDouble(stringArray[i]);
                listOfLatLon.add(new LatLon(lat, lon));
            } else {
                lat = Double.parseDouble(stringArray[i]);
            }
