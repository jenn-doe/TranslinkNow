...
public class StopManager implements Iterable<Stop> {
    ...
    public Stop getStopWithNumber(int number) {
        ...
        else {Stop stop = new Stop(number, " ", new LatLon(-49.2, 123));
    ...
    public Stop getStopWithNumber(int number, String name, LatLon locn) {
        ...
        else {Stop stop = new Stop(number, name, locn);
            stopMap.put(number, stop);
    ...
    public int getNumStops() {
        ...
        return StopManager.getInstance().stopMap.size();
    }
    ...
    public void clearStops() {
        ...
        StopManager.getInstance().stopMap.clear();
    ...
    public Stop findNearestTo(LatLon pt) {
        ...
        double smallestDist = RADIUS;

        for (Stop next : stopMap.values()) {
            double distance = SphericalGeometry.distanceBetween(next.getLocn(), pt);
            if (SphericalGeometry.distanceBetween(next.getLocn(), pt) < smallestDist) {
                closestStopSoFar = next;
                smallestDist = SphericalGeometry.distanceBetween(next.getLocn(), pt);}
